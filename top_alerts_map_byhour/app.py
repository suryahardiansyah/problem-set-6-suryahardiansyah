...
df = pd.read_csv(app_dir / "top_alerts_map_byhour.csv")
...
app_ui = ui.page_sidebar(
    ...
    title="Waze Alerts by Hour Dashboard",
    fillable=True,
)
...
def server(input, output, session):
    @reactive.calc
    def filtered_df():
        # Filter data based on the selected type-subtype and hour
        selected = input.type_subtype()
        selected_hour = f"{input.hour():02}:00"  # Format hour as HH:00
    ...
    def alert_scatter():
        ...
        # Get top locations for the selected type-subtype and hour
        ...
        scatter_plot = alt.Chart(top_locs).mark_point().encode(
            ...
            height=400,
            width=400,
            title=f"{input.type_subtype()} Alerts at {input.hour():02}:00"
        )
